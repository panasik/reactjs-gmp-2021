...
jest.mock('../../../../store', () => ({
  ...
  selectGenres: jest
    .fn()
    .mockReturnValue(['Genre1', 'Genre2', 'Genre3', 'Genre4', 'Genre5']),
  addFilm: jest.fn(),
  loadFilms: jest.fn(),
...
jest.mock('./add-edit-film-form', () => ({
  // eslint-disable-next-line react/prop-types
  onSave, film,
}) => (
  <div>
    <div onClick={() => onSave(film || {})}>Save</div>
  </div>
));
...
describe('AddEditFilmDialog', () => {
  ...
  it('should correctly add new film', () => {
    addFilm.mockImplementation((f, s) => {
      s(mockDispatch);
    });
  ...
  it('should correctly update film', () => {
    updateFilm.mockImplementation((f, s) => {
      s(mockDispatch);
    });
