...
describe('AddEditFilmForm', () => {
  ...
  it('should correctly create form on add', () => {
    render(<AddEditFilmForm onSave={() => {}} onClose={() => {}} />);
  ...
  it('should correctly create form and map film fields', async () => {
    ...
    const posterPath = screen.getByLabelText(/Movie Poster URL/i);
    ...
    expect(posterPath.value).toBe(initFilm.poster_path);
  ...
  it('should correctly update film fields', async () => {
    ...
    await userEvent.type(posterPath, updatedFilm.poster_path);
